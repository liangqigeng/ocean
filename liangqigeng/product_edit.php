<?php
    ...
    $pro_id = $_GET['pro_id'];
    $data = $db->select_one('product','*',"pro_id = $pro_id ORDER BY pro_ord ASC");
    if ($_POST) {
        //接收数据
        $data = array(
        ...
        if ($_FILES['pro_img1']['size']) {
            //先删除旧图片,查询旧图片的地址
            $old_arr = $db->select_one('product','pro_img1',"pro_id = $pro_id");
            //旧图片的图片名称
        ...
        if ($_FILES['pro_img2']['size']) {
            //先删除旧图片,查询旧图片的地址
            $old_arr = $db->select_one('product','pro_img2',"pro_id = $pro_id");
//            echo $old_arr;
            //旧图片的图片名称
        ...
        //执行修改函数
        $res = $db->edit('product',$data, "pro_id = $pro_id");
        if ($res) {
    ...
    $cat = $db->select_all('cat_pro','*','1 ORDER BY cat_id ASC');
?>
	 ...
	 <div class="row">
        ...
        <form action="#" method="post" class="cmxform" enctype="multipart/form-data">
            ...
            <div class="panel-body">
                ...
                <div class="form-group">
                  <div class="input-group"> <span class="input-group-addon">产品尺寸</span>
                      <input type="text" name="pro_size" value="<?php echo $data['pro_size'];?>" class="form-control">
                  </div>
                </div>
                <div class="form-group">

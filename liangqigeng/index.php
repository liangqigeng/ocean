<?php
    ...
    //查询各种数据总数
    //文章总数
    $a_sql = "SELECT COUNT(*) AS c FROM oc_new";
    $a_count = $db->get_one($a_sql);
    //产品总数
    $p_sql = "SELECT COUNT(*) AS c FROM oc_product";
  	$p_count = $db->get_one($p_sql);
  	//留言总数
  	$u_sql = "SELECT COUNT(*) AS c FROM oc_guestbook";
  	$u_count = $db->get_one($u_sql);
 ?>

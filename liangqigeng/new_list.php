...
//            echo $where;
        ...
        //带条件查询总数的SQL语句
        $count = $db->select_count('new',"$where");
        //带条件查询数据的SQL语句
         $new = $db->join_all('new','cat_new','cat_id','a.*, b.cat_name',"$where ORDER BY a.new_addtime ASC LIMIT $con, $limit");
//        echo $sql;die;
    } else {
        //没有搜索的情况
        $new_title = '搜索标题关键词';
        ...
        //不带条件的查询数据的SQL语句
        $new = $db->join_all('new','cat_new','cat_id','a.*, b.cat_name',"1 ORDER BY a.new_addtime ASC LIMIT $con, $limit");
    }
    ...
    if ($_POST && !empty($_POST['idarr'])) {
        ...
        //批量删除
        if ($btn ==1) {
                $res = $db->del('new', "new_id IN($idstr)");
                if ($res) {
        ...
        if ($btn==2) {
            ...
            if (!empty($cat_id)) {
                ...
                $res = $db->edit('new', $data, "new_id IN($idstr)");
                if ($res) {
                  ...
                  <table class="table table-striped table-bordered table-hover dataTable">
                      ...
                      <?php foreach($new as $v) {?>
                          <tr class="success">
                              ...
                              <td><img src="../upload/<?php echo $v['new_photo'];?>" alt="" height="50px"></td>

<?php
    ...
    //通过这个主键做为条件查询当前详细数据
    $banner = $db->select_one('banner','*',"banner_id = $banner_id");
    ...
    if ($_POST) {
        ...
        if ($_FILES['banner_path']['size']) {
            //先删除旧图片,查询旧图片的地址
            $old_arr = $db->select_one('banner','banner_path,thumb_path',"banner_id = $banner_id");
            //旧图片的图片名称
...
//            echo $thumb_path;die;
        ...
        //执行编辑函数
        $res = $db->edit('banner', $data, "banner_id = $banner_id");
        if ($res) {

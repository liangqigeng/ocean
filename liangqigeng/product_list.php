...
//            echo $where;
        ...
        //带条件查询总数的SQL语句
        $count = $db->select_count('product',"$where");

        //带条件查询数据的SQL语句
        $product = $db->join_all('product','cat_pro','cat_id','a.*, b.cat_name',"$where ORDER BY a.pro_addtime DESC LIMIT $con, $limit");
//        echo $sql;die;
    } else {
        //没有搜索的情况
        $pro_name = '搜索标题关键词';
        ...
        //不带条件的查询数据的SQL语句
        $product = $db->join_all('product','cat_pro','cat_id','a.*, b.cat_name',"1 ORDER BY a.pro_addtime DESC LIMIT $con, $limit");
    }
    //执行查询总数的查询数据的SQL 语句
  ...
  if (!empty($_GET['pro_id'])) {
    $del_id = $_GET['pro_id'];
    //先删除旧图片,查询旧图片的地址
        $old_arr = $db->select_one('product','pro_img1',"pro_id = $del_id");
        //旧图片的图片名称
    ...
    $res = $db->del('product', "pro_id = $del_id");
    if ($res) {
...
//批量删除
    if ($btn == 1) {
        $res = $db->del('product', "pro_id IN($idstr)");
        if ($res) {
...
//批量修改留言
    if ($btn == 2) {
        ...
        if (!empty($pro_name)) {
            ...
            $res = $db->edit('product', $data, "pro_id IN($idstr)");
             if ($res) {

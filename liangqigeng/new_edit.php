...
if ($_POST) {
    ...
    if ($_FILES['new_photo']['size']) {
        //先删除旧图片,查询旧图片的地址
        $old_arr = $db->select_one('new','new_photo',"new_id = $new_id");
        //旧图片的图片名称
        $old_path = $old_arr['new_photo'];
        //旧图片完整地址
        ...
        $str = upload('new_photo');
        $arr = explode(',', $str);
        if ($arr[0] == '图片上传成功') {
            $data['new_photo'] = $arr[1];
        } else {
            show_msg($arr[0]);
        }
    }
    $res = $db->edit('new', $data,"new_id = $new_id");
    if ($res) {
        ...
        <form action="#" method="post" class="cmxform" enctype="multipart/form-data">
            ...
            <div class="panel-body">
                ...
                <div class="form-group">
                 ...
                 <div class="form-group">
                      <div class="input-group"> <span class="input-group-addon">新闻封面</span>
                          <input type="file" name="new_photo" value="" class="form-control" id="upload">
                          <label for="upload" style="margin-bottom:0;">
                            <?php if(!empty($data['new_photo'])) {?>
                                <img src="../upload/<?php echo $data['new_photo'];?>" alt="" id="img">
                            <?php } else {?>
                      ...
                      <div class="form-group">
                          <div class="input-group"> <span class="input-group-addon">新闻摘要</span>
                              <input type="text" name="new_summary" value="<?php echo $data['new_summary'];?>" class="form-control">
                          </div>
                      </div>
                      <div class="form-group">

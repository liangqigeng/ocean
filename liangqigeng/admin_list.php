<?php
    ...
    if (!empty($_GET['admin_name'])) {
        ...
        //带条件查询总数的SQL语句
         $count = $db->select_count('admin',$where);
        //带条件查询数据的SQL语句
        $data = $db->select_all('admin','*',"$where ORDER BY admin_addtime DESC LIMIT $con, $limit");
    } else {
        //没有搜索的情况
        $admin_name = '搜索用户名关键词';
        //不带条件的查询总数的SQL语句
        $count = $db->select_count('admin');
        //不带条件的查询数据的SQL语句
        $data = $db->select_all('admin','*',"1 ORDER BY admin_addtime DESC LIMIT $con, $limit");
    }
    ...
    if (!empty($_GET['del_id'])) {
        $del_id = $_GET['del_id'];
        $res = $db->del('admin', "admin_id = $del_id");
        if ($res) {
            show_msg('删除成功', 'admin_list.php');
        } else {
            show_msg('数据执行有误，请重试...');
        }
    }
...
//批量删除
    if ($btn == 1) {
        $res = $db->del('admin', "admin_id IN($idstr)");
        if ($res) {
                          ...
                          <tr class="success">
                              ...
                              <td>
                                  <div class="btn-group">
                                      ...
                                      <a onclick="return confirm('确定要删除吗？');" href="?del_id=<?php echo $v['admin_id'];?>" class="btn btn-default btn-gradient dropdown-toggle"><span class="glyphicons glyphicon-trash"></span></a>
                                  </div>
                              </td>
                          </tr>
                      <?php }?>
